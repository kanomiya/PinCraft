...
public class PinCraftAPI
{
    ...
    public static final List<IPin> loadedPins = Lists.newArrayList();
     ...
     */
    @Nullable
    public static IPin toPin(UUID uuid)
    {
        if (uuid == null) return null;

        Iterator<IPin> itr = loadedPins.iterator();

        while (itr.hasNext())
        {
            IPin pin = itr.next();

            if (pin.getUUID() == null) continue;
            if (pin.getUUID().equals(uuid))
            {
                return pin;
            }
        }
    ...
    public static enum PinCraftAPIEventHandler
    {
        ...
        protected void loadPinHolder(IPinHolder holder)
        {
            ...
            holder.pinSet().stream().forEach((pin) ->
            {
                // IPin pin = pinRef.getPin();
                // UUID uuid = pin.getUUID();

                if (! loadedPins.contains(pin)) loadedPins.add(pin);
            });
        }
